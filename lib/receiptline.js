...
(function () {
    ...
    function wrapText(column, printer) {
        ...
        column.text.forEach((text, i) => {
            ...
            if (i % 2 === 0) {
                ...
                const matchPP = text.match(/##(.+?)##/gm, '')
                let ppLength = 0
                if(matchPP){
                  ppLength = matchPP[0].length
                }
                space = space + ppLength
                ...
                while (t.length > 0) {
                    ...
                    if (w > space || space === 0) {
                        ...
                        if(matchPP){
                          t  = [...Array.from(matchPP[0]), ...t]
                          space = column.width + ppLength;
                        }
                        else{
                          space = column.width
                        }
    ...
    const _svg = {
        ...
        text: function (text, encoding) {
            ...
            const postProcessingRegex = /##(.+?)##/gm
            const matchText = text.match(postProcessingRegex)
            let postProcessingPrefix
            if(matchText) {
              postProcessingPrefix = matchText[0]
              text = text.replace(postProcessingPrefix, '')
            }
            ...
            this.textElement += `<text${attr} ${postProcessingPrefix? `data="${postProcessingPrefix}"`: ''}>${text.replace(/[ &<>]/g, r => ({' ': '&#xa0;', '&': '&amp;', '<': '&lt;', '>': '&gt;'}[r]))}</text>`;
